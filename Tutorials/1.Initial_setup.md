...
```
.vscode   → for formatting settings
```
...
## **2. Default Error Handler — Explanation + Code**
...
# ⭐ **How Error Handlers Are Called from `app.js` (Brief Explanation)**
...
Request → Routes → Match?
          ...
          ├── Yes → Normal Response
          │
          └── No → (Express moves to the next middleware) notFoundHandler 
                    → next(error) 
                    → (Express automatically invokes any middleware defined with 4 parameters) errorHandler 
                    → error.ejs
```
...
# 🟥 **PROBLEM FACED: Access to 127.0.0.1 Was Denied (HTTP ERROR 403)**
...
(Or 3000, 4000, 8000 — anything except 5000.)